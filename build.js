...
// 당첨판매점 데이터(stores.json) 읽어 회차에 부착
let STORES_MAP = {};
try { STORES_MAP = JSON.parse(fs.readFileSync(path.join(__dirname, "stores.json"), "utf8")); } catch (e) { STORES_MAP = {}; }
...
function luckyPage() {
  ...
  const agg = {};
  games.forEach((g) => {
    (g.winStores || []).forEach((s) => {
      const key = s.name + "|" + s.addr;
      if (!agg[key]) agg[key] = { name: s.name, addr: s.addr, r1: 0, r2: 0, total: 0, types: {} };
      const a = agg[key];
      if (s.rank === 1) a.r1++; else if (s.rank === 2) a.r2++;
      a.total++;
      a.types[g.typeName] = (a.types[g.typeName] || 0) + 1;
    });
  });
  ...
  const list = rows.map((s, i) => {
    ...
    const q = encodeURIComponent(`${s.name} ${s.addr}`);
    const badges = (s.r1 ? `<span class="wcb c1">1등 ${s.r1}</span>` : "") + (s.r2 ? `<span class="wcb c2">2등 ${s.r2}</span>` : "");
    return `<a class="lrow" href="https://map.naver.com/p/search/${q}" target="_blank" rel="noopener">
...
<style>
  ...
  .wcb.c1{background:var(--amber)}.wcb.c2{background:var(--slate)}
...
<body>
  ...
  <div class="wrap">
    <section class="hero" style="padding:44px 0 8px">
      <span class="eyebrow">🏆 전 회차 당첨판매점 집계</span>
      <h1 style="font-size:clamp(28px,5.4vw,48px)">스피또 당첨 명당 순위</h1>
      <p>스피또2000·1000·500에서 1·2등이 많이 나온 판매점을 모았어요. 가게를 누르면 지도로 연결됩니다.</p>
    </section>
    <div class="lstats">
    ...
    ${adUnit()}
    <div class="section-h"><h2>당첨 횟수 TOP ${rows.length}</h2><span class="desc">1등 → 전체 당첨 순</span></div>
    <div class="llist">${list || '<p class="empty">집계된 당첨판매점이 아직 없어요.</p>'}</div>

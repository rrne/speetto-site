...
// 당첨판매점 데이터: 역대 아카이브 + 최근(fresh) 병합 (fresh가 최신값으로 덮음)
let STORES_MAP = {};
...
const TYPE_META = {
  SP2000: {
    ...
    title: "스피또2000 출고율 실시간 조회·당첨율 | 1등 10억 | 긁",
  ...
  SP1000: {
    ...
    title: "스피또1000 출고율 실시간 조회·당첨율 | 1등 5억 | 긁",
  ...
  SP500: {
    ...
    title: "스피또500 출고율 실시간 조회·당첨율 | 1등 2억 | 긁",
...
const GUIDES = [
  {
    slug: "guide-shipment", cat: "출고율", emoji: "📦",
    title: "스피또 출고율 보는 법 — 뜻부터 유리한 회차 고르기 | 긁",
    desc: "스피또 출고율이 무엇인지, 왜 중요한지, 출고율과 1등 잔여를 함께 보고 유리한 회차를 고르는 법까지. 스피또2000·1000·500 출고율 실시간 조회 가이드.",
    keywords: "스피또 출고율, 스피또 출고율 보는법, 스피또 출고율 뜻, 출고율이란, 스피또2000 출고율, 스피또1000 출고율, 스피또 출고율 조회",
    h1: "스피또 출고율, 제대로 보는 법",
    lead: "스피또를 살 때 ‘어떤 회차가 유리할까?’의 핵심 힌트가 바로 <b>출고율</b>이에요. 출고율이 뭔지, 왜 중요한지, 그리고 출고율로 회차를 고르는 법까지 한 번에 정리했어요.",
    body:
      "<h2>출고율이란?</h2>" +
      "<p>출고율은 <b>발행된 스피또 복권 중 판매점으로 출고(입고)된 비율</b>입니다. 예를 들어 출고율이 70%라면 전체 발행량의 70%가 이미 시중 판매점에 풀렸다는 뜻이에요. 출고율이 100%면 그 회차 물량이 모두 시중에 나온 상태입니다.</p>" +
      "<h2>출고율을 왜 봐야 하나?</h2>" +
      "<p>스피또는 추첨식 로또와 달리 <b>각 회차에 당첨 복권이 이미 정해져 인쇄·발행</b>됩니다. 즉 1등이 몇 장 들어있는지 정해져 있고, 시간이 지나며 누군가 그 1등을 긁어가죠. 그래서 핵심은 <b>‘1등이 아직 시중에 남아있는가’</b>예요.</p>" +
      "<p>여기서 출고율이 단서가 됩니다. <b>출고율은 높은데(많이 풀렸는데) 1등은 아직 많이 남아있다면</b>, 안 나온 1등이 시중 어딘가에 있을 가능성이 그만큼 크다고 볼 수 있어요.</p>" +
      "<h2>출고율만 보면 안 된다 — 1등 잔여와 함께</h2>" +
      "<p>출고율 하나만으로는 부족해요. 반드시 <b>등위별 잔여 당첨(특히 1등이 몇 매 남았는지)</b>과 함께 봐야 합니다.</p>" +
      "<ul class=\"glist\">" +
      "<li><b>출고율 높음 + 1등 많이 남음</b> → 유리 (안 나온 1등이 시중에 있을 확률↑)</li>" +
      "<li><b>출고율 높음 + 1등 거의 빠짐</b> → 불리 (큰 게 이미 나감)</li>" +
      "<li><b>출고율 낮음(초기 회차)</b> → 1등이 통째로 남아있을 수 있으나 아직 시중 물량이 적음</li>" +
      "</ul>" +
      "<h2>출고율 실시간으로 보는 법</h2>" +
      "<p>동행복권 사이트에서 회차별로 일일이 확인할 수도 있지만, 회차마다 들어가 보기 번거롭습니다. ‘긁?’은 <b>스피또2000·1000·500의 회차별 출고율과 1·2·3등 잔여 당첨</b>을 한 페이지에 모아 실시간으로 보여주고, 출고율 대비 1등이 많이 남은 회차를 <b>추천 지수</b>로 정렬해줘요.</p>" +
      "<p>" +
      "<a class=\"idetail\" href=\"/\">전체 회차 출고율 한눈에 보기 →</a> " +
      "<a class=\"idetail\" href=\"/2000\">스피또2000 출고율</a> " +
      "<a class=\"idetail\" href=\"/1000\">스피또1000 출고율</a> " +
      "<a class=\"idetail\" href=\"/500\">스피또500 출고율</a>" +
      "</p>" +
      "<h2>정리: 유리한 회차 고르는 순서</h2>" +
      "<ol class=\"glist\"><li>출고율이 어느 정도 올라온(시중에 많이 풀린) 회차를 본다</li><li>그중 <b>1등 잔여율이 높은</b> 회차를 고른다</li><li>추천 지수가 높은 회차를 우선 후보로</li></ol>" +
      "<p style=\"color:var(--muted);font-size:13.5px\">※ 출고율·잔여 당첨은 확률을 높이는 참고 지표일 뿐, 당첨을 보장하지 않습니다. 복권은 본인 판단과 책임하에, 과도한 구매는 삼가세요.</p>",
    faq: [
      ["스피또 출고율이 무슨 뜻인가요?", "발행된 복권 중 판매점으로 출고된 비율입니다. 출고율 70%면 전체 물량의 70%가 시중에 풀렸다는 뜻이고, 100%면 모두 풀린 상태입니다."],
      ["출고율이 높으면 좋은 건가요?", "출고율이 높은데도 1등이 많이 남아있다면, 안 나온 1등이 시중에 있을 가능성이 커 유리하다고 볼 수 있습니다. 출고율은 반드시 1등 잔여와 함께 봐야 합니다."],
      ["스피또 출고율은 어디서 보나요?", "동행복권 공식 발행내역에서 확인할 수 있고, ‘긁?’(ge-uk.com)에서 스피또2000·1000·500 회차별 출고율과 잔여 당첨을 실시간으로 한눈에 볼 수 있습니다."],
      ["출고율이 100%면 사면 안 되나요?", "출고율 100%여도 1등이 아직 남아있다면 살 만합니다. 반대로 출고율이 높고 1등이 다 빠졌다면 그 회차는 메리트가 적습니다."],
    ],
  },
  {
...
// ============================================================
//  당첨 명당 페이지 — 전 회차 1·2등 당첨판매점 집계 랭킹
// ============================================================
function luckyPage() {
  const url = "https://ge-uk.com/lucky-stores";
  const normRegion = (r) => { if (!r) return "기타"; return r.replace(/특별자치도|특별자치시|특별시|광역시|도$/, "").trim() || r; };
  const agg = {};
  ...
  const all = Object.values(agg).sort((a, b) => b.r1 - a.r1 || b.total - a.total);
  const rows = all.slice(0, 200);
  const totalShops = all.length;
  const total1 = all.reduce((a, s) => a + s.r1, 0);
  const total2 = all.reduce((a, s) => a + s.r2, 0);
  // 지역 필터 칩 (표시 rows 기준)
  const regionCnt = {};
  rows.forEach((s) => { regionCnt[s.region] = (regionCnt[s.region] || 0) + 1; });
  const regions = Object.entries(regionCnt).sort((a, b) => b[1] - a[1]);
  const chips = `<button class="rchip on" data-region="">전체 (${rows.length})</button>` +
    regions.map(([r, c]) => `<button class="rchip" data-region="${r}">${r} (${c})</button>`).join("");

  const ld = `{"@context":"https://schema.org","@graph":[
    {"@type":"BreadcrumbList","itemListElement":[
      {"@type":"ListItem","position":1,"name":"긁?","item":"https://ge-uk.com/"},
      {"@type":"ListItem","position":2,"name":"당첨 명당","item":${JSON.stringify(url)}}]},
    {"@type":"WebPage","url":${JSON.stringify(url)},"name":"스피또 당첨 명당 순위","inLanguage":"ko-KR"}]}`;

  const list = rows.map((s, i) => {
    const no = i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : (i + 1);
...
<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>스피또 당첨 명당 순위 — 1·2등 많이 나온 판매점 | 긁</title>
<meta name="description" content="스피또2000·1000·500에서 1·2등이 가장 많이 나온 당첨 판매점(명당) 순위. 전 회차 당첨판매점을 집계해 당첨 횟수와 지도 링크로 보여드립니다." />
<meta name="keywords" content="스피또 명당, 복권 명당, 스피또 당첨 판매점, 스피또 1등 명당, 복권 1등 명당, 당첨 많은 복권방, 스피또2000 명당" />
<meta name="robots" content="index,follow,max-image-preview:large" />
<meta name="theme-color" content="#0071e3" />
<link rel="canonical" href="${url}" />
<meta name="google-adsense-account" content="${ADSENSE}" />
...
<meta property="og:type" content="website" />
<meta property="og:site_name" content="긁?" />
<meta property="og:title" content="스피또 당첨 명당 순위 | 긁" />
<meta property="og:description" content="1·2등이 가장 많이 나온 스피또 당첨 판매점 순위 — 당첨 횟수·지도 링크." />
<meta property="og:url" content="${url}" />
<meta property="og:image" content="https://ge-uk.com/og-image.png" />
<meta name="twitter:card" content="summary_large_image" />
<meta name="twitter:title" content="스피또 당첨 명당 순위 | 긁" />
<meta name="twitter:description" content="1·2등이 많이 나온 스피또 판매점 순위." />
<script type="application/ld+json">${ld}</script>
${STYLE}
<style>
  .lstats{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin:24px 0 8px}
  .lstat{background:var(--surface);border-radius:16px;box-shadow:var(--shadow-sm);padding:18px;text-align:center}
  .lstat .v{font-size:30px;font-weight:800;letter-spacing:-.03em}.lstat .k{font-size:13px;color:var(--muted);font-weight:700;margin-top:4px}
  .llist{display:grid;gap:9px;margin-top:8px}
  .lrow{display:grid;grid-template-columns:48px 1fr auto;align-items:center;gap:13px;background:var(--surface);
    border-radius:14px;box-shadow:var(--shadow-sm);padding:14px 16px;text-decoration:none;transition:.15s}
  .lrow:hover{box-shadow:var(--shadow);transform:translateY(-2px)}
  .lmain{min-width:0}
  .lbadges{display:flex;gap:6px;margin-top:6px;flex-wrap:wrap}
  .wcb{font-size:11.5px;font-weight:800;border-radius:999px;padding:3px 9px;color:#fff}
  .wcb.c1{background:var(--amber)}.wcb.c2{background:var(--slate)}.wcb.cr{background:var(--bg);color:var(--ink2)}
  .rchips{display:flex;gap:8px;flex-wrap:wrap;margin:4px 0 14px}
  .rchip{border:0;cursor:pointer;font-family:inherit;font-size:13px;font-weight:700;color:var(--ink2);
    background:var(--surface);box-shadow:var(--shadow-sm);padding:8px 14px;border-radius:999px;transition:.15s}
  .rchip:hover{color:var(--ink)} .rchip.on{background:var(--ink);color:#fff}
...
<body>
  ...
  <div class="wrap">
    <section class="hero" style="padding:44px 0 8px">
      <span class="eyebrow">🏆 역대 전 회차 당첨판매점 집계</span>
      <h1 style="font-size:clamp(28px,5.4vw,48px)">스피또 당첨 명당 순위</h1>
      <p>스피또2000·1000·500 역대 회차에서 1·2등이 많이 나온 판매점을 모았어요. 지역으로 필터하고, 가게를 누르면 지도로 연결됩니다.</p>
    </section>
    <div class="lstats">
      <div class="lstat"><div class="v">${fmt(totalShops)}</div><div class="k">집계된 당첨 판매점</div></div>
      <div class="lstat"><div class="v">${fmt(total1)}</div><div class="k">1등 당첨 건수</div></div>
      <div class="lstat"><div class="v">${fmt(total2)}</div><div class="k">2등 당첨 건수</div></div>
    </div>
    ${adUnit()}
...
// 지역 필터
(function(){
  var chips=document.getElementById('rchips'), rows=[].slice.call(document.querySelectorAll('#llist .lrow'));
  if(!chips) return;
  chips.addEventListener('click',function(e){
    var b=e.target.closest('.rchip'); if(!b) return;
    chips.querySelectorAll('.rchip').forEach(function(x){x.classList.toggle('on',x===b);});
...
fs.writeFileSync(path.join(__dirname, "lucky-stores.html"), luckyPage(), "utf8");
fs.writeFileSync(path.join(OUT_DIR, "lucky-stores.html"), luckyPage(), "utf8");
console.log("당첨 명당 페이지 생성: /lucky-stores");
